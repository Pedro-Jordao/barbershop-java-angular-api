...
public class ClientQueryService implements IClientQueryService {
    ...
    public ClientEntity findById(final long id) {
        return repository.findById(id).orElseThrow(
                () -> new NotFoundException("Não foi encontrado o cliente de id %s".formatted(id))
        );
    }
    ...
    @Override
    public void verifyPhone(long id, final String phone) {
    ...
    public void verifyEmail( final String email) {
        var optional= repository.existsByEmail(email);
        if (optional) {
    ...
    @Override
    public void verifyEmail(long id, final String email) {
